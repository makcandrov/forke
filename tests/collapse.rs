...
#[test]
fn long_cascade_stays_shallow() {
    // A spine of dead two-child nodes. Removing the spare child bottom-up
    // makes each spine node collapsible in turn, so the whole spine is torn
    ...
    let mut spares = Vec::with_capacity(SPINE);

    let mut current = root.fork(vec![1]);
    for _ in 0..SPINE {
        let next = current.fork(vec![1]);
        spares.push(current.fork(vec![1]));
        spine.push(current);
        current = next;
    }
    let tip = current;

    // Every spine node is now dead with two children, so nothing collapses.
    drop(spine);
    drop(tip);
...
fn randomized_drop_orders_never_double_merge() {
    ...
    for _ in 0..ROUNDS {
        ...
        observers.push(obs);
        let mut nodes = vec![Node::root(data)];

        for _ in 0..24 {
            let (data, obs) = Tracked::pair();
            observers.push(obs);
            let i = rng.random_range(0..nodes.len());
            let child = nodes[i].fork(data);
            nodes.push(child);
        }
        ...
        for obs in &observers {
            assert!(obs.is_dropped(), "payload {} outlived the tree", obs.id());
        }
        ...
        merged.dedup();
        assert_eq!(total, merged.len(), "a payload was merged into two places");
    }
}
